<aside id="sidebar" class="collapse collapse-horizontal border-end">
    ...
    <div class="h-100">
        ...
        <ul class="sidebar-nav">
            ...
            {{-- Dashboard --}}
            <li class="sidebar-item">
                <a href="{{ route('admins.dashboard') }}" class="sidebar-link {{ request()->routeIs('home') ? 'active' : '' }}" wire:navigate>
            ...
            {{-- ✅ Trash Link — Jobs ke bilkul neeche --}}
            <li class="sidebar-item">
                <a href="{{ route('job-posts.trash') }}" wire:navigate
                   class="sidebar-link d-flex align-items-center gap-2
                          {{ request()->routeIs('job-posts.trash') ? 'active' : '' }}"
                   style="padding-left:2.2rem; font-size:13px; color:#ffffff;">

                    <i class="bi bi-trash3" style="font-size:13px;"></i>
                    <span>Trash</span>

                    @php
                        $jobTrashCount   = \App\Models\JobPost::onlyTrashed()->count();
                        $notifTrashCount = \Illuminate\Support\Facades\DB::table('notifications')
                                             ->where('notifiable_id', auth()->id())
                                             ->whereNotNull('deleted_at')
                                             ->count();
                        $totalTrash = $jobTrashCount + $notifTrashCount;
                    @endphp


                    @if($totalTrash > 0)
                        <span class="badge ms-auto"
                              style="background:#fee2e2;color:#b91c1c;font-size:10px;font-weight:700;">
                            {{ $totalTrash }}
                        </span>
                    @endif
                </a>
                    ...
                    <li class="sidebar-item">
                        <a href="{{ route('job-posts.index') }}?status=upcoming"
                           class="sidebar-link d-flex align-items-center gap-2">
                            ...
                            Upcoming
                            @php $upcoming = \App\Models\JobPost::where('auction_status','pending')->count(); @endphp
                            @if($upcoming > 0)
                                <span class="badge ms-auto" style="background:#fef3c7;color:#b45309;">{{ $upcoming }}</span>
                            @endif
                        </a>
                    </li>
                    ...
                    <li class="sidebar-item">
                        <a href="{{ route('job-posts.index') }}?status=live"
                           class="sidebar-link d-flex align-items-center gap-2">
                            ...
                            Live
                            @php $live = \App\Models\JobPost::where('auction_status','open')->count(); @endphp
                            @if($live > 0)
                                <span class="badge ms-auto" style="background:#dcfce7;color:#15803d;">{{ $live }}</span>
                            @endif
                        </a>
                    </li>
                    ...
                    <li class="sidebar-item">
                        <a href="{{ route('job-posts.index') }}?status=under_verification"
                           class="sidebar-link d-flex align-items-center gap-2">
                            ...
                            @endphp
                            @if($underVerif > 0)
                                <span class="badge ms-auto" style="background:#f5f3ff;color:#7c3aed;">{{ $underVerif }}</span>
                            @endif
                        </a>
                    </li>
                    ...
                    <li class="sidebar-item">
                        <a href="{{ route('job-posts.index') }}?status=closed"
                           class="sidebar-link d-flex align-items-center gap-2">
                            ...
                            @endphp
                            @if($closed > 0)
                                <span class="badge ms-auto" style="background:#f3f4f6;color:#374151;">{{ $closed }}</span>
                            @endif
                        </a>
                    </li>
            ...
            <li class="sidebar-item">
                <a href="#" class="sidebar-link collapsed" data-bs-target="#reports" data-bs-toggle="collapse"
                   aria-expanded="false">
                    <i class="bi bi-file-earmark-bar-graph pe-2"></i>
                    Reports
                </a>
                <ul id="reports" class="sidebar-dropdown list-unstyled collapse" data-bs-parent="#reports-sidebar">
                    <li class="sidebar-item">
                        <a href="{{ route('job-accepted-declined.index') }}" wire:navigate
                           class="sidebar-link {{ request()->routeIs('job-accepted-declined.index') ? 'active' : '' }}">
            ...
            <li class="sidebar-item">
                ...
                <ul id="billing" class="sidebar-dropdown list-unstyled collapse" data-bs-parent="#sidebar">
                    <li class="sidebar-item">
                        <a href="{{ route('billing.subscriptions.index') }}" wire:navigate
                           class="sidebar-link {{ request()->routeIs('billing.subscriptions.index') ? 'active' : '' }}">
                            Subscriptions
                        </a>
                    </li>
                    <li class="sidebar-item">
                        <a href="{{ route('billing.payment-requests.index') }}" class="sidebar-link" wire:navigate>
                            Payment Request
                        </a>
                    </li>
                    <li class="sidebar-item">
                        <a href="{{ route('billing.payments.index') }}" class="sidebar-link" wire:navigate>
                            Payments
                        </a>
                    </li>
                </ul>
            ...
            <li class="sidebar-item border-top border-light">
                <a href="{{ route('settings.index') }}" wire:navigate class="sidebar-link">
