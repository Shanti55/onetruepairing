...
@section('content')
    ...
    <div class="px-3">
        <div class="d-flex align-items-center justify-content-between">
            <h4 class="fw-semibold">Page <i class="bi bi-chevron-right"></i> Contact Us</h4>
        </div>
    </div>

   @include('admin-panel.settings.contact-us._form')
...
@push('js')
    <script type="module">
        $(function () {
            ...
            //Store
            $('#contactUsSettingForm').on('submit', function (e) {
                e.preventDefault();

                var data = new FormData($('#contactUsSettingForm')[0]);

                $.easyAjax({
                    url: "{{ route('settings.storeOrUpdate') }}",
                    container: '#contactUsSettingForm',
                    type: "POST",
                    disableButton: true,
                    blockUI: true,
                    data: data,
